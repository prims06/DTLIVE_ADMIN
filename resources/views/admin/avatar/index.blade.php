                        ...
                        <div class="modal-footer justify-content-center">
                            <form id="save_avatar_sortable" enctype="multipart/form-data">
                                ...
                                <div class="w-100 text-center">
                                    <button type="button" class="btn btn-default mw-120" onclick="save_avatar_sortable()">{{__('label.save')}}</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <script>
            ...
            if(Demo_Mode == 1){
                ...
                $.ajax({
                    ...
                    success:function(resp){
                        $("#dvloader").hide();
                        get_responce_message(resp, 'avatar', '{{ route("admin.avatar.index") }}');
                    },
        ...
        function change_status(id, status) {
            ...
            if(Demo_Mode == 1){
                ...
                $("#dvloader").show();
                var url = "{{route('admin.avatar.show', '')}}" + "/" + id;
                $.ajax({
                    type: "GET",
                    url: url,
                    headers: {
            ...
            if(Demo_Mode == 1){
                ...
                $.ajax({
                    ...
                    success: function(resp) {
                        ...
                        get_responce_message(resp, 'edit_avatar', '{{ route("admin.avatar.index") }}');
                    },
                    error: function(XMLHttpRequest, textStatus, errorThrown) {
                        $("#dvloader").hide();
                        toastr.error(errorThrown, textStatus);
        ...
        function sortableBTN() {
            ...
            for (var i = 0; i < avatar_data.length; i++) { 
                htmlContent += '<div id="'+ avatar_data[i].id +'" class="listitemClass mb-3 sortablebox">' +
                                '<p class="m-2 py-1">'+ avatar_data[i].name +'</p>' +
                            '</div>';
            }
